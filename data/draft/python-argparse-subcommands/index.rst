...
не нужны, если бы была возможность цепочек вызовов:

.. code:: py

    subparsers.add_parser('run').add_argument('port').set_defaults(func=run_server)

Но в argparse цепочек вызовов не получится, хотя может в каких-то случаях использования 
они не нужны, но в моей практике чаще хочется цепочек.
...
А вот вывод help для определенной подкоманды отличается отсутствием описаний и коротких 
аналогов для параметров::
...
пояснений, то использовать argh очень заманчиво тем более он в принципе позволяет и
...
проект минималистичный или небольшой скрипт с подкомандами. Еще в улучшаторах присутствует 
немного магии, argparse же прямой как двери.
